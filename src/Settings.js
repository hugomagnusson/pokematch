...
function Settings() {
    ...
    const [values, setValues] = useState({minGen: state.minGen, maxGen: state.maxGen});
    const [oldSpritesEnabled, setOldSpritesEnabled] = useState(state.oldSprites);
    const [saveEnabled, setSaveEnabled] = useState(true);

    useEffect(() => {
        if(values.minGen === state.minGen && values.maxGen === state.maxGen && oldSpritesEnabled === state.oldSprites){
    ...
    return (
        <Container>
            <h2>Settings</h2>
            <DoubleRangeSlider setValuesLOL={setValuesFormat} defaultValues={[values.minGen, values.maxGen]} min={MIN} max={MAX} />
            <Form>
                <Form.Group as={Row}>
                ...
                <Button variant="primary" onClick={handleSaveSettings} disabled={!saveEnabled}>
                    Save Settings
                </Button>
            </Form>
        </Container>
